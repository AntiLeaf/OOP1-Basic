...
const string traceFolder = "..\\Test\\trace\\";
...
void clearTempFiles() {
    int r = system(
        "if exist test_ans del test_ans; if exist test_out del test_out");
    (void)r;
}

int testTrace(const char *trace) {
    clearTempFiles();
    if (system((string() + standerBasic + "<"+trace + " > test_ans")
                   .c_str()) != 0) {
        return 1;
    }

    if (system((string() + studentBasic + "<" + trace + " > test_out")
                   .c_str()) != 0)
        return 2;
    if (system("fc test_ans test_out")!=0) return 4;
    // clearTempFiles();
    return 0;
}
    ...
    if (!error) {
        ...
        if (!silent) {
            ...
            if (!hideError) {
                ...
                cout.flush();
                int r0 = system(
                    (string() + "type " + currentTrace).c_str());
                (void)r0;
                ...
                if (error == 4) {
                    ...
                    cout.flush();
                    int r1 = system("type test_ans");
                    (void)r1;
                    cout << color("\x1b[0m") << endl;
                    cout << "Your output: " << endl << color("\x1b[33m");
                    cout.flush();
                    int r2 = system("type test_out");
                    (void)r2;
                    cout << color("\x1b[0m") << endl;
                }
            }
